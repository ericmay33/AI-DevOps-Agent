...
def main():
    ...
    parser.add_argument(
        '--analyze',
        action='store_true',
        help='Run failure analysis after gathering context'
    )
            ...
            try:
                ...
                if analysis.fix_steps:
                    ...
                    for i, step in enumerate(analysis.fix_steps, 1):
                        print(f"  {i}. [{step.action.value}] {step.file}")
                        print(f"     {step.change}")
                
                # Save analysis
                analysis_output = args.output.replace('.json', '_analysis.json')
